...
const MOCKS = {
  'status.cgi': {
    all: { ok:true, data:{
      ipt_running:true,   ipt_pid:212,
      proxy_running:false, proxy_pid:null,
      wpa_running:false,  wpa_pid:null,
      wlan1_up:false, wlan1_state:'DISCONNECTED', wlan1_ip:'',
      cap_running:false, rule_count:2,
      uptime:'3h 22m', cache_free:'18.4 MB', data_free:'174.6 MB',
      kernel:'3.18.48-perf-g7c5d8a2'
    }}
  },
  ...
  'diag.cgi': {
    status: { ok:true, data:{
      ...
      stream_port: 37026,
      qcmap: { running:true, pid:789 },
      mask: {
        lte_rrc:true, lte_nas:true, lte_l1:false, lte_mac:false,
        lte_rlc:false, lte_pdcp:false, nr_rrc:true, wcdma:true,
        gsm:true, umts_nas:true, ip_data:true, f3_debug:false,
        gps:false, qmi_events:false
      }
    }},
  ...
  'firewall.cgi': {
    list: { ok:true, data:[
      { num:1, table:'nat',    chain:'PREROUTING', target:'REDIRECT',
        src:'0.0.0.0/0', dst:'0.0.0.0/0',
        opts:'tcp dpt:80 redir ports 8118', friendly:'Redirect port 80 \u2192 8118' },
      { num:2, table:'filter', chain:'INPUT',      target:'DROP',
        src:'192.168.1.105', dst:'0.0.0.0/0',
        opts:'', friendly:'Block 192.168.1.105' }
    ]}
  },
  ...
  'wifi.cgi': {
    status: { ok:true, data:{
      wpa_running:false, wpa_pid:null, wpa_state:'DISCONNECTED',
      ssid:'', ip_address:'',
      networks:[
        { id:0, ssid:'HomeNetwork', current:false },
        { id:1, ssid:'LabAP',       current:false }
      ],
      raw:''
    }}
  },
  ...
  'capture.cgi': {
    status: { ok:true, data:{
      ...
      ifaces: ['rmnet_data0', 'wlan0', 'wlan1', 'lo'],
      files:  []
    }}
  }
};
...
function serveMock(res, cgiName, action) {
  ...
  const data = cgi[action] || cgi[Object.keys(cgi)[0]] || { ok:false, error:'no mock for '+cgiName+'/'+action };
...
function serveStatic(res, urlPath) {
  ...
  const filePath = path.resolve(path.join(WWW, rel));
...
const server = http.createServer((req, res) => {
  ...
  if (pathname.startsWith('/cgi-bin/')) {
    const cgiName = path.basename(pathname);
    if (req.method === 'POST') {
      let body = '';
      req.on('data', c => body += c);
      req.on('end', () => {
        const params = Object.fromEntries(new URLSearchParams(body));
        serveMock(res, cgiName, params.action || parsed.searchParams.get('action') || '');
      });
    } else {
